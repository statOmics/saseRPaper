...
.initial_beta <- function(se, orthonormal_vector_space, offset = NULL){
  ...
  log_data <- log((counts(se)+0.5))-log(exp(offset)+1)
...
.adapted_IRLS <- function(se, initial_beta, reduced_orthonormal_vector_space, fit_DGE){
  
  mu <- exp(t(reduced_orthonormal_vector_space%*%t(initial_beta)) + log(exp(fit_DGE$offset)+1))
