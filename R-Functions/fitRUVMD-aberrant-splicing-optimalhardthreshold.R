...
findEncodingDimRUVSplicing <- function(se,
  ...
  assay(se,'offsets', withDimnames = FALSE) <- real_offsets 
...
fitRUVMD <- function(se, dimensions = NULL, padjust = "BH", BPPARAM = bpparam()){
  ...
  svd_latent_factors <- SingularVectors$u[,1:dimensions]
...
.fitRUV <- function(se, orthonormal_vector_space, dimensions, beta_initial, fit_DGE){
  ...
  mu <- .adapted_IRLS_splicing(se = se,
...
.adapted_IRLS_splicing <- function(se, initial_beta, reduced_orthonormal_vector_space, fit_DGE, maxiter=50){
  ...
  for(i in c(1:maxiter)){
    ...
    mu <- pmin(exp(t(reduced_orthonormal_vector_space%*%beta) + fit_DGE$offset)*0.2 + 0.8* mu,
               matrix(c(exp(fit_DGE$offset)), ncol = ncol(se),nrow = nrow(se)))
  }
...
OffsetSplicing <- function(se){
  ...
  offsets <- merge(list("locus" = rowData(se)$locus),offsets, by = "locus")
