...
saveFraserDataSet(fds = fds, dir = paste0(data_directory,
                "/Geuvadis/Fraser"),
                         name = "Geuvadis_Fraser_PCA_jaccard_filtered_counts")
fds <- loadFraserDataSet(dir = paste0(data_directory,
                "/Geuvadis/Fraser"),
                         name = "Geuvadis_Fraser_PCA_jaccard_filtered_counts")
...
```{r}
start_time <- Sys.time()
fds <- calculatePSIValues(fds, BPPARAM = MulticoreParam(1), types ="jaccard")
...
saveRDS(object = time_filtering, 
        file = paste0(output_directory,"/Time_filtering_Fraser_PCA_Jaccard.RDS"))
...
fds <- optimHyperParams(fds, type = "jaccard", plot = F, q_param = seq(2,11,1), BPPARAM = MulticoreParam(1))
...
saveRDS(object = time_hyperparameter_optimalisation, 
        file = paste0(output_directory,"/Time_hyperparameter_optimalisation_Fraser_PCA_Jaccard.RDS"))

saveFraserDataSet(fds = fds, 
                  dir = 
                paste0(output_directory,"/Fraser"),
                  name = "Geuvadis_optimal_hyperparameters_Jaccard")
...
saveRDS(object = time_analysis, 
        file = paste0(output_directory,"/Time_analysis_Fraser_PCA_Jaccard.RDS"))

saveRDS(object = time_total, 
        file = paste0(output_directory,"/Time_Fraser_PCA_total_Jaccard.RDS"))
