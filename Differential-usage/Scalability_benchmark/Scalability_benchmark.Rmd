```{r}
#!/usr/bin/env Rscript
# root.dir <- paste0("./Differential-usage/Scalability_benchmark")
# setwd(root.dir)
root.dir <- getwd()
print(root.dir)
...
# 2. Scalability benchmark of egdeR_offset wiht estimateDisp
...
for (i in L$groupSizes){
	for (j in L$transcripts_amount){
		...
		edgeR_offset_estDisp_1[round] <- list(result) # runtimes stored in a list
...
save(edgeR_offset_estDisp_1, file = "./Results/edgeR_offset_estDisp_1.RData") # I save this list as RData
...
for (i in L$groupSizes){
	for (j in L$transcripts_amount){
		...
		print(paste("Two groups of", i, "cells each with", j, "transcripts"))
		run_edgeR_offset(L, countData, tx2gene, estimateDisp = TRUE) #function outputs the object result
		edgeR_offset_estDisp_2[round] <- list(result) # runtimes stored in a list
...
save(edgeR_offset_estDisp_2, file = "./Results/edgeR_offset_estDisp_2.RData") # I save this list as RData
