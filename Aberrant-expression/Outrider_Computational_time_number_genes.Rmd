...
```{r, analysis}
set.seed(100)
samples_used <- sample(c(1:dim(rds)[1]), size = dim(rds)[1], replace = F)

rds <- estimateSizeFactors(rds)

time_usage <- vector(length = length(c(number_of_genes)))
names(time_usage) <- c(number_of_genes)


for(i in c(1:length(c(number_of_genes)))){
  start_time <- Sys.time()

  rds_loop <- OUTRIDER(ods = rds[samples_used[1:number_of_genes[i]],],
