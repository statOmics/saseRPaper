...
```{r}
df_plot_3 <- df_time %>% filter(method %in% c("saseR-bins",
                                      "saseR-ASpli",
                                      "saseR-junctions",
                                      "FRASER-PCA 2.0",
                                      "FRASER-BB-Decoder 2.0",
                                      "FRASER-Autoencoder 2.0"))
df_plot_3$method <- factor(df_plot_3$method, levels = c("saseR-bins",
                                      "saseR-junctions",
                                      "saseR-ASpli",
                                      "FRASER-Autoencoder 2.0",
                                      "FRASER-BB-Decoder 2.0",
...
```{r}
time_labels <- c("6 min","1 min", "2 min", "1.7 h", "13.2 h", "20.1 h")
plot_3 <- ggplot(data = df_time %>% filter(method %in% c("saseR-bins",
                                      "saseR-ASpli",
                                      "saseR-junctions",
                                      "FRASER-PCA 2.0",
                                      "FRASER-BB-Decoder 2.0",
  ...
  scale_fill_manual(values = c("saseR-bins" = "#E69F00", 
                                "saseR-ASpli"="#D55E00", 
                                "saseR-junctions" = "#CC79A7",
                                "FRASER-PCA 2.0"= "#009E73", 
                                "FRASER-BB-Decoder 2.0" = "#56B4E9",
...
```{r}
ggsave(paste0(output_directory,"/Figure-4.png"), Figure_1, 
       device = "png", width = 9, height = 4.5, dpi = 300)
ggsave(paste0(output_directory,"/Figure-4.eps"), Figure_1, 
       device = cairo_ps, width = 9, height = 4.5, dpi = 300)
ggsave(paste0(output_directory,"/Figure-4.pdf"), Figure_1, 
       device = "pdf", width = 9, height = 4.5, dpi = 300)

```


# Supplementary figure LeafcutterMD and SPOT

```{r}


plot_extra <- ggplot(data = df_results %>% filter(method %in% c("saseR-bins",
                                      "saseR-ASpli",
                                      "saseR-junctions",
                                      "FRASER-PCA 2.0",
                                      "FRASER-BB-Decoder 2.0",
...
```{r}
plot_extra_2 <- ggplot(data = df_pr%>% filter(type == "PRC" & (modname %in% c("saseR-bins",
                                "saseR-ASpli",
                                "saseR-junctions",
                                "FRASER-Autoencoder 2.0",
                                      "FRASER-BB-Decoder 2.0",
                                ...
                                "LeafcutterMD",
                                "SPOT"))), 
                       aes(x = x, y = y, col = modname)) + geom_line() + ggtitle(label = "") + ylab("Precision")  + xlab("Recall") + theme_bw() + 
  theme(axis.text=element_text(size=10), axis.title=element_text(size=12))  + scale_color_manual(values = 
                              c("saseR-bins" = "#E69F00", 
                                "saseR-ASpli"="#D55E00", 
                                "saseR-junctions" = "#CC79A7",
...
```{r}
legend <- get_legend(plot_extra_2)
prow <- plot_grid(plot_extra + theme(legend.position = "none"), plot_extra_2 + theme(legend.position = "none")
                  , labels = c("A","B"), ncol = 2,
                  rel_heights = c(1,1), align = "h")
Figure_extra <- plot_grid(prow,legend,ncol = 1, rel_heights= c(1,0.1))
Figure_extra

```

```{r}
ggsave(paste0(output_directory,"/Supplementary-Figure-extra-splicing-methods.png"), Figure_extra, 
       device = "png", width = 12, height = 6, dpi = 300)
ggsave(paste0(output_directory,"/Supplementary-Figure-extra-splicing-methods.eps"), Figure_extra, 
       device = cairo_ps, width = 12, height = 6, dpi = 300)
ggsave(paste0(output_directory,"/Supplementary-Figure-extra-splicing-methods.pdf"), Figure_extra, 
       device = "pdf", width = 12, height = 6, dpi = 300)

```

# Supplementary figure FRASER comparison

```{r}

plot_4 <- ggplot(data = df_results %>% filter(method %in% c("FRASER-PCA psi[3]",                                   
                                                 "FRASER-BB-Decoder psi[3]",
                                                 "FRASER-Autoencoder psi[3]",
                                                 "FRASER-PCA psi[5]",
                                                 "FRASER-BB-Decoder psi[5]",
                                                 "FRASER-Autoencoder psi[5]",
                                                  ...
                                                  "FRASER-Autoencoder 2.0")), 
                 aes(x = method, y = auc, col = method)) + stat_summary(fun.data = bp.vals, geom="boxplot") + geom_jitter(size = 0.2) + 
  scale_color_manual(values = c("FRASER-PCA psi[3]" = "burlywood1",                                   
                                "FRASER-BB-Decoder psi[3]"= "coral",
                                "FRASER-Autoencoder psi[3]"= " red3",
                                "FRASER-PCA psi[5]"= "cyan1",
                                "FRASER-BB-Decoder psi[5]"= "seagreen1",
                                "FRASER-Autoencoder psi[5]"= "lightcyan4", 
                                "FRASER-PCA 2.0"= "#009E73", 
                                "FRASER-BB-Decoder 2.0" = "#56B4E9",
                                "FRASER-Autoencoder 2.0" = "#0072B2")) + 
  ylab("Area under the curve per sample") + theme_bw()  + ylim(c(0,1.0001)) + theme(legend.position = "none")+
...
```{r}
labels <- expression(paste("FRASER-Autoencoder ", psi[3]), 
                     paste("FRASER-BB-Decoder 2.0"),
                     paste("FRASER-Autoencoder ", psi[5]), 
                     paste("FRASER-PCA ", psi[3]),
                     paste("FRASER-Autoencoder 2.0"), 
                     paste("FRASER-PCA ", psi[5]),
                     paste("FRASER-BB-Decoder ", psi[3]), 
                     paste("FRASER-PCA 2.0"),
                     paste("FRASER-BB-Decoder ", psi[5]))


plot_5 <- 
  ggplot(data = df_pr%>% filter(type == "PRC" & (modname %in% 
                                                   c("FRASER-PCA psi[3]",                                  
                                "FRASER-BB-Decoder psi[3]",
                                "FRASER-Autoencoder psi[3]",
                                "FRASER-PCA psi[5]",
                                ...
                                "FRASER-PCA 2.0", 
                                "FRASER-BB-Decoder 2.0",
                                "FRASER-Autoencoder 2.0"))), aes(x = x, y = y, col = modname)) + geom_line() + ggtitle(label = "") + ylab("Precision")  + xlab("Recall") + theme_bw() + 
  theme(axis.text=element_text(size=12), axis.title=element_text(size=14))  + 
  scale_color_manual(values = c("FRASER-PCA psi[3]" = "burlywood1",                   
                                "FRASER-BB-Decoder psi[3]"= "coral",
                                "FRASER-Autoencoder psi[3]"= "red3",
                                ...
                                "FRASER-Autoencoder 2.0" = "#0072B2"), 
                     labels = labels,
                     breaks = c("FRASER-Autoencoder psi[3]", 
                                "FRASER-BB-Decoder 2.0",
                                "FRASER-Autoencoder psi[5]",
                                "FRASER-PCA psi[3]",
                                "FRASER-Autoencoder 2.0", 
                                ...
                                "FRASER-BB-Decoder psi[5]"))+ 
  theme( legend.title = element_blank(),legend.position = "bottom", 
         legend.text = element_text(size = 11)) + ylim(c(0,0.25))
print(plot_5)
...
```{r}
legend <- get_legend(plot_5)
prow <- plot_grid(plot_4 + theme(legend.position = "none"), plot_5 + theme(legend.position = "none")
                  , labels = c("A","B"), ncol = 2,
                  rel_heights = c(1,1), align = "h")
...
```{r}
ggsave(paste0(output_directory,"/Supplementary-Figure-6.png"), Figure_2, 
       device = "png", width = 12, height = 6, dpi = 300)
ggsave(paste0(output_directory,"/Supplementary-Figure-6.eps"), Figure_2, 
       device = cairo_ps, width = 12, height = 6, dpi = 300)
ggsave(paste0(output_directory,"/Supplementary-Figure-6.pdf"), Figure_2, 
       device = "pdf", width = 12, height = 6, dpi = 300)

```


# Supplementary figure with saseR jaccard

```{r}
df_plot_4 <- df_results %>% filter(method %in% c("saseR-bins",
                                      "saseR-ASpli",
                                      "saseR-junctions",
                                      "saseR-jaccard",
                                      "FRASER-PCA 2.0",
...
df_plot_4$method <- factor(df_plot_4$method, levels = c("saseR-bins",
                                      "saseR-junctions",
                                      "saseR-ASpli",
                                      "saseR-jaccard",
                                      "FRASER-BB-Decoder 2.0",
...
plot_4 <- ggplot(data = df_plot_4, aes(x = method, y = auc, col = method)) + stat_summary(fun.data = bp.vals, geom="boxplot") + geom_jitter(size = 0.2) + 
  scale_color_manual(values = c("saseR-bins" = "#E69F00", 
                                "saseR-ASpli"="#D55E00", 
                                "saseR-junctions" = "#CC79A7",
...
```{r}
legend <- get_legend(plot_2)
prow <- plot_grid(plot_4 + theme(legend.position = "none"), plot_2 + theme(legend.position = "none")
                  , labels = c("A","B"), ncol = 2,
                  rel_heights = c(1,1), align = "h")
Figure_3 <- plot_grid(prow,legend,ncol = 1, rel_heights= c(1,0.1))
Figure_3

```

```{r}
ggsave(paste0(output_directory,"/Supplementary-Figure-11.png"), Figure_3, 
       device = "png", width = 9, height = 4.5, dpi = 300)
ggsave(paste0(output_directory,"/Supplementary-Figure-11.eps"), Figure_3, 
       device = cairo_ps, width = 9, height = 4.5, dpi = 300)
ggsave(paste0(output_directory,"/Supplementary-Figure-11.pdf"), Figure_3, 
       device = "pdf", width = 9, height = 4.5, dpi = 300)
